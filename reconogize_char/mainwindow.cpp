...
void MainWindow::test1()///初始采集
{
    for(int i=0;i<26;i++)
    {
        char str[10];
        sprintf(str,"\jpg\\%c1.jpg",i+'a');
        Mat src=imread(str);
...
{
    Mat src=imread("f:/num\\a.jpg");
    Mat src1(src.rows,src.cols,src.type());
    for(int m=0;m<src.rows;m++)
        for(int n=0;n<src.cols;n++)
            for(int i=0;i<3;i++)
                src1.at<Vec3b>(m,n)[i]=src.at<Vec3b>(m,n)[i];
//    this->study(src,src1,'a');
    this->test(src,src1);
}
void MainWindow::study(Mat src,Mat src1, char num)
{
...
void MainWindow::write_file(char i,double temp[])
{
    char str[20];
    sprintf(str,"\src\\%c.txt",i);
    FILE *p;
    p=fopen(str,"ab");
